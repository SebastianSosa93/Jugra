...
//obtiene la información de cada juego en la base de datos.
...
async function insertUsuario(nombre,apellido,email,password,rol){
    useDB("jugra");
    const errorOriginal = console.error;
    console.error = function(){}
    try{
        return await database.sql(`INSERT INTO Usuarios(nombre,apellido,email,password,rol) VALUES('${nombre}','${apellido}','${email}','${password}','${rol}')`);
    }catch(error){
        if (error.message.includes('UNIQUE constraint failed')) {
...
async function modificarRol(usuarioID, rol){
    useDB("jugra");
    return await database.sql(`UPDATE Usuarios
                        SET rol = '${rol}'
